...
`,pr=`
  ...
  skewVarianceCrossed
  liquidityVarianceCrossed
`;var me;(function(n){n[n.FifteenMinutes=900]="FifteenMinutes",n[n.OneHour=3600]="OneHour",n[n.FourHours=14400]="FourHours",n[n.EightHours=28800]="EightHours",n[n.OneDay=86400]="OneDay",n[n.SevenDays=604800]="SevenDays"})(me||(me={}));async function Se(n,e){return n.query({...e,errorPolicy:"all"})}const pu=re`
...
`;async function ou(n,e){var t;const{data:a}=await Se(n.govSubgraphClient,{query:pu,variables:{user:e.toLowerCase()}});return(t=a==null?void 0:a.claimAddeds.map(i=>({amount:M.from(i.amount),blockNumber:i.blockNumber,claimer:i.claimer,epochTimestamp:parseInt(i.epochTimestamp),rewardToken:i.rewardToken,timestamp:i.timestamp,tag:i.tag})).filter(i=>i.rewardToken.toLowerCase()!=="0xCb9f85730f57732fc899fb158164b9Ed60c77D49".toLowerCase()))!==null&&t!==void 0?t:[]}const uu=re`
...
`;async function Uu(n,e,t){var a;const i=e.board(),r=e.block.timestamp,s=Math.min(i.expiryTimestamp,(a=t==null?void 0:t.endTimestamp)!==null&&a!==void 0?a:r),o=(await bt(n,Hu,{optionId:`${e.market().address.toLowerCase()}-${e.strike().id}-${e.isCall?"call":"put"}`},{...t,endTimestamp:s})).map(l=>({optionPrice:M.from(l.optionPrice),blockNumber:l.blockNumber,timestamp:l.timestamp})),u={optionPrice:e.price,blockNumber:e.block.number,timestamp:s};return[...o,u].filter(l=>l.optionPrice.gt(0))}const Wu=re`
...
`;async function ll(n,e,t){var a,i,r,s;const p=E(n,e.contractAddresses,n.version,F.OptionToken),[o,u]=await Promise.allSettled([p.getPositionWithOwner(t),Se(n.subgraphClient,{query:ul,variables:{positionId:t,market:e.address.toLowerCase()}})]),l=o.status==="fulfilled"?o.value:null,y=u.status==="fulfilled"?u.value:null,m=(a=y==null?void 0:y.data)===null||a===void 0?void 0:a.positions[0],d=(i=m==null?void 0:m.trades.map(Pa))!==null&&i!==void 0?i:[],T=(r=m==null?void 0:m.collateralUpdates.map(ga))!==null&&r!==void 0?r:[],c=(s=m==null?void 0:m.transfers.map(Ca))!==null&&s!==void 0?s:[],h=m!=null&&m.settle?wa(m.settle):null;if(l){const v=l.strikeId.toNumber(),k=Ma(l.optionType),b=e.liveOption(v,k);return xr(l.owner,l,b,d,T,c,h)}else{if(m)return Ar(m,e,d,T,c,h);throw new Error("Failed to fetch position")}}const yl=re`
...
`,gl={tvl:f,freeLiquidity:f,burnableLiquidity:f,utilization:0,reservedCollatLiquidity:f,pendingDeltaLiquidity:f,usedDeltaLiquidity:f,tokenPrice:f,pendingDeposits:f,pendingWithdrawals:f};async function wl(n,e){if(e.liveBoards().length===0)return{tvl:e.params.NAV,freeLiquidity:e.params.NAV,burnableLiquidity:f,utilization:0,reservedCollatLiquidity:f,pendingDeltaLiquidity:f,usedDeltaLiquidity:f,tokenPrice:e.params.tokenPrice,pendingDeposits:f,pendingWithdrawals:f,timestamp:e.block.timestamp};const{data:t}=await Se(n.subgraphClient,{query:kl,variables:{market:e.address.toLowerCase()}});if(!t||t.marketTotalValueSnapshots.length===0)return{...gl,timestamp:e.block.timestamp};const a=t.marketTotalValueSnapshots[0],i=M.from(a.freeLiquidity),r=M.from(a.burnableLiquidity),s=M.from(a.NAV),p=M.from(a.usedCollatLiquidity),o=M.from(a.pendingDeltaLiquidity),u=M.from(a.usedDeltaLiquidity),l=M.from(a.tokenPrice);return{timestamp:a.timestamp,freeLiquidity:i,burnableLiquidity:r,tvl:s,utilization:s.gt(0)?q(s.sub(i).mul(S).div(s)):0,reservedCollatLiquidity:p,pendingDeltaLiquidity:o,usedDeltaLiquidity:u,tokenPrice:l,pendingDeposits:M.from(a.pendingDeposits),pendingWithdrawals:M.from(a.pendingWithdrawals)}}const Pl=re`
...
`,Cl={poolNetDelta:f,hedgerNetDelta:f,netDelta:f,netStdVega:f};async function Ml(n,e){const{data:t}=await Se(n.subgraphClient,{query:Pl,variables:{market:e.address.toLowerCase()}});if(!t||t.marketGreeksSnapshots.length===0)return{...Cl,timestamp:e.block.timestamp};const a=t.marketGreeksSnapshots[0],i=M.from(a.poolNetDelta),r=M.from(a.hedgerNetDelta),s=M.from(a.netDelta),p=M.from(a.netStdVega);return{poolNetDelta:i,hedgerNetDelta:r,netDelta:s,netStdVega:p,timestamp:a.timestamp}}const Ol=re`
  query marketTotalValueSnapshots(
...
`,Sl={freeLiquidity:f,burnableLiquidity:f,tvl:f,utilization:0,reservedCollatLiquidity:f,pendingDeltaLiquidity:f,usedDeltaLiquidity:f,tokenPrice:f,pendingDeposits:f,pendingWithdrawals:f};async function xl(n,e,t){var a;const i=await bt(n,Ol,{market:e.address.toLowerCase()},{...t,endTimestamp:(a=t==null?void 0:t.endTimestamp)!==null&&a!==void 0?a:e.block.timestamp});return i.length===0?[{...Sl,timestamp:e.block.timestamp}]:i.map(s=>{const p=M.from(s.freeLiquidity),o=M.from(s.burnableLiquidity),u=M.from(s.NAV),l=M.from(s.usedCollatLiquidity),y=M.from(s.pendingDeltaLiquidity),m=M.from(s.usedDeltaLiquidity),d=M.from(s.tokenPrice);return{freeLiquidity:p,burnableLiquidity:o,tvl:u,utilization:u.gt(0)?q(u.sub(p).mul(S).div(u)):0,totalWithdrawingDeposits:f,reservedCollatLiquidity:l,pendingDeltaLiquidity:y,usedDeltaLiquidity:m,tokenPrice:d,timestamp:s.timestamp,pendingDeposits:M.from(s.pendingDeposits),pendingWithdrawals:M.from(s.pendingWithdrawals)}})}async function Il(n,e){return await E(n,e,n.version,F.OptionMarket).owner()}const Al=re`
...
`,Fl={poolNetDelta:f,hedgerNetDelta:f,netDelta:f,netStdVega:f};async function ql(n,e,t){var a;const i=await bt(n,Al,{market:e.address.toLowerCase()},{...t,endTimestamp:(a=t==null?void 0:t.endTimestamp)!==null&&a!==void 0?a:e.block.timestamp});return i.length===0?[{...Fl,timestamp:e.block.timestamp}]:i.map(r=>{const s=M.from(r.poolNetDelta),p=M.from(r.hedgerNetDelta),o=M.from(r.netDelta),u=M.from(r.netStdVega);return{poolNetDelta:s,hedgerNetDelta:p,netDelta:o,netStdVega:u,timestamp:r.timestamp}})}const Vl={currentPositions:{longPosition:{size:f,collateral:f,averagePrice:f,entryFundingRate:f,unrealisedPnl:f,lastIncreasedTime:f,isLong:!0},shortPosition:{size:f,collateral:f,averagePrice:f,entryFundingRate:f,unrealisedPnl:f,lastIncreasedTime:f,isLong:!1},amountOpen:f,isLong:!0},futuresPoolHedgerParams:{acceptableSpotSlippage:f,deltaThreshold:f,marketDepthBuffer:f,targetLeverage:f,maxLeverage:f,minCancelDelay:f,minCollateralUpdate:f,vaultLiquidityCheckEnabled:!1},hedgerAddresses:{router:Pe,positionRouter:Pe,vault:Pe,quoteAsset:Pe,baseAsset:Pe,weth:Pe},gmxView:{basePoolAmount:f,baseReservedAmount:f,quotePoolAmount:f,quoteReservedAmount:f,maxGlobalLongSize:f,guaranteedUSD:f,maxGlobalShortSize:f,shortSize:f,minExecutionFee:f,remainingLongDollars:f,remainingShortDollars:f},referralCode:"",pendingOrderKey:"",lastOrderTimestamp:f,spotPrice:f,expectedHedge:f,currentHedge:f,currentLeverage:f,pendingCollateralDelta:f,baseBal:f,quoteBal:f,wethBal:f},Dl={interactionDelay:f,hedgeCap:f};async function Ll(n){const e=te(n,A.Newport,Q.OptionMarketViewer),t=te(n,A.Newport,Q.ExchangeAdapter),a={contract:t,function:"owner",args:[]},i=await jt(n),r=ge(n)?[]:i.map(g=>({contract:E(n,g,A.Newport,F.PoolHedger),function:"getHedgerState",args:[]})),s=i.map(g=>({contract:t,function:"getAdapterState",args:[g.optionMarket]})),p=ge(n)?[]:i.map(g=>({contract:E(n,g,A.Newport,F.PoolHedger),function:"getPoolHedgerParams",args:[]})),o=i.map(g=>({contract:E(n,g,A.Newport,F.LiquidityPool),function:"getTokenPrice",args:[]})),{returnData:[u,l,...y],blockNumber:m}=await tt(n,[a,{contract:e,function:"getMarkets",args:[i.map(g=>g.optionMarket)]},...r,...s,...p,...o]),d=y.slice(0,r.length),T=y.slice(r.length,r.length+s.length),c=y.slice(r.length+s.length,r.length+s.length+p.length),h=y.slice(r.length+s.length+p.length),{isPaused:v,markets:k}=l;return{marketViews:k.map((g,C)=>({marketView:g,hedgerView:ge(n)?Vl:d[C],adapterView:T[C],poolHedgerParams:ge(n)?Dl:c[C],tokenPrice:h[C],baseLimit:null})),isGlobalPaused:v,owner:u,blockNumber:m}}const Bl={interactionDelay:f,hedgeCap:f},El={lastInteraction:f,hedgedDelta:f,margin:f,leverage:f,hedgerQuoteBalance:f,hedgerMarginQuoteBalance:f,canHedgeDeltaIncrease:!0,canHedgeDeltaDecrease:!0,cappedExpectedHedge:f,snxHasEnoughMarketDepth:!0,marketSuspended:!1,curveRateStable:!0,pendingDeltaLiquidity:f,usedDeltaLiquidity:f,pendingDelta:f,pendingMargin:f,fundingRate:f,trackingCode:"",optionMarket:"",perpsMarket:"",curveSwap:"",quoteAsset:"",futuresPoolHedgerParams:{targetLeverage:f,maximumFundingRate:f,deltaThreshold:f,marketDepthBuffer:f,priceDeltaBuffer:f,worstStableRate:f,maxOrderCap:f},poolHedgerParams:{interactionDelay:f,hedgeCap:f},longInterest:f,shortInterest:f,maxTotalMarketSize:Ht};async function _l(n){const e=te(n,A.Newport,Q.OptionMarketViewer),t=te(n,A.Newport,Q.ExchangeAdapter),a={contract:t,function:"owner",args:[]},i=await jt(n),r=ge(n)?[]:i.map(P=>({contract:E(n,P,A.Newport,F.PoolHedger),function:"getHedgerState",args:[]})),s=ge(n)?[]:i.map(P=>({contract:t,function:"getAdapterState",args:[P.optionMarket]})),p=ge(n)?[]:i.map(P=>({contract:E(n,P,A.Newport,F.PoolHedger),function:"getPoolHedgerParams",args:[]})),o=ge(n)?[]:i.map(P=>({contract:E(n,P,A.Newport,F.LiquidityPool),function:"getTokenPrice",args:[]})),u=i.map(P=>({contract:E(n,P,A.Newport,F.OptionMarket),function:"baseLimit",args:[]})),{returnData:[l,y,...m],blockNumber:d}=await tt(n,[a,{contract:e,function:"getMarkets",args:[i.map(P=>P.optionMarket)]},...r,...s,...p,...o]),{returnData:T}=await tt(n,u),c=m.slice(0,r.length),h=m.slice(r.length,r.length+s.length),v=m.slice(r.length+s.length,r.length+s.length+p.length),k=m.slice(r.length+s.length+p.length),{isPaused:b,markets:g}=y;return{marketViews:g.map((P,O)=>({marketView:P,hedgerView:ge(n)?El:c[O],adapterView:h[O],poolHedgerParams:ge(n)?Bl:v[O],tokenPrice:k[O],baseLimit:T[O]})),isGlobalPaused:b,owner:l,blockNumber:d}}const Fn=1e4,Rl=re`
...
`;async function Nl(n,e,t){var a,i,r;const s=(a=t==null?void 0:t.startTimestamp)!==null&&a!==void 0?a:Ji,p=(i=t==null?void 0:t.endTimestamp)!==null&&i!==void 0?i:e.block.timestamp,o=(r=t==null?void 0:t.period)!==null&&r!==void 0?r:hr(s,p,[me.FifteenMinutes,me.OneHour,me.FourHours,me.EightHours,me.OneDay,me.SevenDays]),u=o>0?(p-s)/o:0,l=Math.ceil(u/Fn),y=await vr(n,Rl,{min:s,max:p,limit:Fn,period:o,market:e.address.toLowerCase()},"timestamp",{increment:Fn*o,batch:l});if(y.length===0)return[];const m=y.map(T=>({open:M.from(T.open),high:M.from(T.high),low:M.from(T.low),close:M.from(T.close),startTimestamp:T.timestamp-T.period,endTimestamp:T.timestamp,period:T.period,startBlockNumber:T.blockNumber})),d=m.length?m[m.length-1]:null;return d&&d.endTimestamp>e.block.number&&(d.close=e.spotPrice,e.spotPrice.lt(d.low)&&(d.low=e.spotPrice),e.spotPrice.gt(d.high)&&(d.low=e.spotPrice)),m}const Gl=re`
